...
export function ChampionInfoContent() {
  ...
  const [infoChampAsync, setInfoChampAsync] = useState({
    splash: "",
    loading: "",
  });
  ...
  const handleSkinsOnleftPartButtons = (event) => {
    if (event.target.classList[0] === "left-arrow") {
      skinIndex === 0
        ? setSkinIndex(championInfo.skins.length - 1)
        : setSkinIndex(skinIndex - 1);
    } else {
      skinIndex === championInfo.skins.length - 1
        ? setSkinIndex(0)
        : setSkinIndex(skinIndex + 1);
    }
  };
  ...
  const handleSkinsName = () => {
    if (skinIndex === 0) return championInfo.name;
      ...
      return infoChampAsync.championName === "default"
        ? "Loading..."
        : infoChampAsync.championName;
  };
  ...
  const buttonsFromPainelLevelLogic = (event) => {
    const buttonTarget = event.path.filter(
      (el) =>
        el.className === "leftPart__lvlPainel__buttonDash" ||
        el.className === "leftPart__lvlPainel__buttonPlus"
    );

    if (buttonTarget[0].className == "leftPart__lvlPainel__buttonPlus")
      ChampionLvl.value >= 18
        ? (ChampionLvl.value = 18)
        : setNumberPerLevel((prevState) => ++prevState);
    else
      ChampionLvl.value <= 1
        ? (ChampionLvl.value = 1)
        : setNumberPerLevel((prevState) => --prevState);
    ...
    if (championInfo) {
      const champInfo = async () => {
        ...
        const championStatsPerLevel = Object.fromEntries(
          championInfo.stats.filter(([key, value]) => key.includes("perlevel"))
        );
        const championStats = Object.fromEntries(
          championInfo.stats.filter(([key, value]) => !key.includes("perlevel"))
        );
        ...
        setInfoChampAsync({
          ...
          stats: [
            [
              "HP",
              championStats.hp +
                numberPerLevel * championStatsPerLevel.hpperlevel,
            ],
            ["Move Speed", championStats.movespeed],
            [
              "MP",
              championStats.mp +
                numberPerLevel * championStatsPerLevel.mpperlevel,
            ],
            [
              "ATK Speed",
              championStats.attackspeed +
                numberPerLevel * championStatsPerLevel.attackspeedperlevel,
            ],
            [
              "ATK",
              championStats.attackdamage +
                numberPerLevel * championStatsPerLevel.attackdamageperlevel,
            ],
            ["ATK Range", championStats.attackrange],
            [
              "AMR",
              championStats.armor +
                numberPerLevel * championStatsPerLevel.armorperlevel,
            ],
            [
              "HP Regen",
              championStats.hpregen +
                numberPerLevel * championStatsPerLevel.hpregenperlevel,
            ],
            [
              "MR",
              championStats.spellblock +
                numberPerLevel * championStatsPerLevel.spellblockperlevel,
            ],
            [
              "MP Regen",
              championStats.mpregen +
                numberPerLevel * championStatsPerLevel.mpregenperlevel,
            ],
          ],
        });
      };
  ...
  return (
    <div
      className="content"
      style={{ backgroundImage: `url(${infoChampAsync.splash})` }}
    >
      <ContentLeftPart
